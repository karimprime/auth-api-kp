...
export class AuthApiKpService extends AuthAPI {
  ...
  constructor() {
    ...
    if (!this._http) {
      throw new Error('HttpClient must be provided. Make sure HttpClientModule is imported.');
    }
    if (!this._config) {
      throw new Error(
  ...
  register(data: SignUpRequest): Observable<SignUpResponse | ErrorResponse> {
    return this._http.post<SignUpResponse>(this.buildUrl('register'), data).pipe(
      tap((res) => {
        if (res.token && res.user) {
          this.updateUserData(res.user);
          this.clearProfileCache();
   ...
   */
  forgetPassword(data: ForgotPasswordRequest): Observable<ForgotPasswordResponse | ErrorResponse> {
    return this._http
   ...
   */
  verifyCode(data: VerifyResetCodeRequest): Observable<VerifyResetCodeResponse | ErrorResponse> {
    return this._http
   ...
   */
  resetPassword(data: ResetPasswordRequest): Observable<ResetPasswordResponse | ErrorResponse> {
    return this._http
   ...
   */
  changePassword(data: ChangePasswordRequest): Observable<ChangePasswordResponse | ErrorResponse> {
    return this._http
   ...
   */
  uploadPhoto(data: UploadPhotoRequest): Observable<UploadPhotoResponse | ErrorResponse> {
    ...
    return this._http.put<UploadPhotoResponse>(this.buildUrl('uploadPhoto'), formData).pipe(
   ...
   */
  getProfileData(forceRefresh = false): Observable<ProfileDataResponse | ErrorResponse> {
    if (!forceRefresh && this._profileDataCache$) {
      return this._profileDataCache$;
    }

    const request$ = this._http.get<ProfileDataResponse>(this.buildUrl('profileData')).pipe(
      tap((res) => {
        this.updateUserData(res.user);
      }),
      shareReplay(1),
   ...
   */
  editProfile(data: EditProfileRequest): Observable<EditProfileResponse | ErrorResponse> {
    return this._http.put<EditProfileResponse>(this.buildUrl('editProfile'), data).pipe(
      tap((res) => {
        this.updateUserData(res.user);
        this.clearProfileCache();
      }),
   ...
   */
  updateRole(data: UpdateRoleRequest): Observable<UpdateRoleResponse | ErrorResponse> {
    return this._http.put<UpdateRoleResponse>(this.buildUrl('editProfile'), data).pipe(
      tap((res) => {
        this.updateUserData(res.user);
        this.clearProfileCache();
      }),
